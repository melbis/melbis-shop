...
The same configuration, but tied to a single store, the program puts into the store's own folder — for applications that open a folder as a project: `.mcp.json` and `.claude\settings.json` are read by Claude Code, `.codex\config.toml` and `AGENTS.md` by Codex. An assistant started from them works **with that store only**, whatever is open in the program: open the store's folder, and the correspondence about different stores does not pile up in one heap. Once the application asks whether you trust this folder and you confirm, the assistant will call its tools without asking permission for every call: its limits are set by your rights in the store. The files are rewritten every time the program starts, so adding anything of your own to them is pointless; they are taken from the `Engine\MCP\Setup\` folder of the distribution.
