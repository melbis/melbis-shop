...
Notes pile up quickly, so from time to time the assistant offers to review them, and it deletes what is no longer needed only with your consent. The business's established rules belong not in notes but in the **[store charter](../charter.md)**. A new store gets a sample charter right away, and the assistant turns to it from the first session; in a store without a charter it will offer to set one up. Who reads the charter and who edits it is decided by the section access rights in the "[Catalog](catalog.md)". The memory is left with the everyday.
