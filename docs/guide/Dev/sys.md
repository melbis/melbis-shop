...
| `SysTopicRight($mUserId, $mTopicId = 0, $mFor = 'frame')` | a section: `frame`, `price`, `ctrl` |
...
- **A super-right** outranks the granted ones: `PUT_OUTSIDE_RIGHT` opens all web
  ...
  info items, `PUT_ORDER_OPTION` — all order option values, `PUT_KEY_VALUE` — all
  settings.
...
**A move into the node's own branch writes nothing** and answers `false`: a node cannot become a descendant of itself.
...
The relation lives until the end of the request.
...
**`SysDependSweep($mTable, $mTries = 10, $mPause = 500)`** — to remove what is already hanging. It walks the whole chain: a dependent may have dependents of its own. It answers with a row **per relation**, not per table:
