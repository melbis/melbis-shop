...
In addition to system configuration parameters, the platform supports custom project constants — arbitrary key-value pairs stored in the database table `{DBNICK}_self_key_value`. This is a convenient mechanism for storing settings specific to a particular project: working hours, limits, feature toggles, and any other values a store's employee edits themselves.
