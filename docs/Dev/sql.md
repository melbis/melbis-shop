...
`SqlTableLock` and `SqlTableUnlock` are **not** `LOCK TABLES` at the DBMS level, but a cooperative "occupied" mark that the application and all modules see. It is needed for coordination with the native client: while the manager is performing a batch operation on products, a background server task must not touch the same tables.
...
**The method waits out an occupied table by itself.** The full signature is `SqlTableLock($mLine, $mTables, $mUserId = 0, $mTries = false, $mPause = false)`: the number of attempts and the pause between them in milliseconds; `false` means 10 attempts with a 500 ms pause. With the defaults, `false` arrives after ten attempts over five seconds — the method sits out somebody else's short piece of work rather than carrying the refusal through to a person. Whoever needs the former instant answer passes a single attempt:
...
Both methods have a third argument — `$mUserId`, the employee the table is taken **for**. It is visible in the application's list of locks, and the lock is released by it as well.
