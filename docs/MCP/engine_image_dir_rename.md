...
* **another parent, the same name** — the folder is moved. Any images or statics folder
  will do, including one in another template group.

The group's own `images/` is the root: it cannot be renamed or moved. The links to the folder's files in the markup and in the styles do not change.

## The Answer

```json
{"path": "./../templates/default/images/banners",
 "new_path": "./../templates/default/images/promo", "bundled": 0}
```
...
| `bundled` | how many of the files inside had a bundle description — see "[Statics](engine_static.md)" |
...
| `No such folder: … engine_map_tree lists the folders of both trees under dirs.` | the new parent is not among the images and statics folders |
| `A folder cannot be moved inside itself.` | the new parent lies inside the folder itself |
| `Folder already exists: <name>` | there is already a folder with this name in the new place |
