...
**An AI tool** is a tool the owner has written for their store: a module and its description in "Development → AI Components". Each of them consists of **commands**. The server runs them with its own MCP tools: `tool_list` shows what there is, `tool_run` carries out one command, `tool_pool` — many commands in one call.
